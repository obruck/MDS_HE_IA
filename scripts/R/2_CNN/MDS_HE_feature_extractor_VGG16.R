...
image_files_path <- "/data/images/3_preprocessed_tiles_grayscale"
...
for (image_dir in dir_list) {
  ...
  save(vgg16_feature_list, file = paste0(output_directory, "/MDS_imagetiles_VGG16_", image_dir_name, ".Rdata"))
...
###################### EXPORT ONLY TOP 2048 FEATURES WITH MOST VARIANCE/MEAN VALUE ######################################################################


# Create WSI label
feature_list$name <- gsub("CLEAN_NAME", "", feature_list$vgg16_feature_list_name)
feature_list <- feature_list %>% dplyr::select(name, vgg16_feature_list_name, everything())
feature_list1 <- feature_list %>%
  dplyr::select(-vgg16_feature_list_name)

# Select numeric data
feature_list_cluster <- feature_list1 %>% dplyr::select(-name)

# Remove variables with only values = 0
feature_list_cluster <- feature_list_cluster[, colSums(feature_list_cluster != 0) > 0]

# Select top 2048 features with highest variance
myvars_var <- apply(feature_list_cluster,2,var,na.rm=TRUE)
myvars_mean <- apply(feature_list_cluster,2,mean,na.rm=TRUE)
myvars <- myvars_var/myvars_mean
myvars <- sort(myvars,decreasing=TRUE)
myvars <- myvars[1:2048]
feature_list_cluster <- feature_list_cluster[,names(myvars)]
feature_list_cluster$name <- feature_list1$name
...
# Save feature list
saveRDS(feature_list_cluster, "Feature_list_VGG16_MDS.rds")
