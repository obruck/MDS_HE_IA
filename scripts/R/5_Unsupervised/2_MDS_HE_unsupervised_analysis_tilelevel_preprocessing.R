...
# Unsupervised learning of MDS HE tile images
...
for (dg_list in dg_list1) {
  for (dg_phase in dg_phase1) {
    if (!z == "First round over!" & y == "Xception") {
      ...
      vgg16_feature_list1 <- vgg16_feature_list
      vgg16_feature_list <- vgg16_feature_list %>% dplyr::select(-Image_name)
      
      ## Aggregate by the image spot name
      if (image_size == "tile") {
        print("Tile data = no aggregation")
      } else if (image_size == "wsi") {
        vgg16_feature_list <- aggregate(.~name, vgg16_feature_list, mean)
        print("WSI data = aggregate by sample")
      } else {
        print("Define tile or WSI data")
      }
    ...
    ## Modify data
    vgg16_feature_list_cluster <- vgg16_feature_list_cluster %>% mutate(
      name = vgg16_feature_list$name)
    vgg16_feature_list_cluster_withname <- vgg16_feature_list_cluster %>% filter(name %in% vgg16_feature_list_cluster_clinical$Image_name)
    vgg16_feature_list_cluster_withname$Image_name1 <- vgg16_feature_list1 %>% filter(name %in% vgg16_feature_list_cluster_clinical$Image_name) %>% dplyr::select(Image_name)
    vgg16_feature_list_cluster_withname$Image_name <- vgg16_feature_list_cluster_withname$name
